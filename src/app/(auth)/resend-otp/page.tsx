...
import { resendOtpSchema } from "@/Schemas/resendOtpSchema";
...
export default function SendVerifyCode() {
  ...
  const form = useForm<z.infer<typeof resendOtpSchema>>({
  ...
  const onSubmit = async (data: z.infer<typeof resendOtpSchema>) => {
    setIsSubmitting(true);
    try {
      const response = await axios.post<ApiRespopnse>("/api/send-verify-code", {
        email: data.email,
  ...
  return (
    <div className={`flex justify-center items-center min-h-screen `}>
      <div className="w-full max-w-lg p-8 space-y-8 rounded-lg">
        <div className="text-center">
          <h1 className="text-4xl font-extrabold tracking-tight lg:text-5xl mb-5">
            Forgot Your Otp
          </h1>
          <p className="mb-4">Enter the details to get a new one</p>
        </div>
        <Form {...form}>
